...
export default function TransferTokensTab() {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingToHolding) {
        ...
        const { error: createError } = await supabase.from('user_token_holdings').insert({
          user_id: toUserId,
    ...
    <div className="max-w-3xl mx-auto">
      <div className="glass-card p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Transfer Tokens</h2>
            <p className="text-sm text-muted-foreground">Move tokens between users (admin operation)</p>
          </div>
        </div>
        ...
        <div className="mb-4 p-3 bg-muted/30 rounded-lg border border-border text-sm text-muted-foreground flex items-start gap-2">
          ...
          <span>
            This is an <strong>admin transfer</strong> that updates database records. For user-initiated P2P transfers,
            direct users to the Transfers page.
          </span>
        </div>
              ...
              <div className="space-y-2">
                ...
                {isLoadingHoldings ? (
                  ...
                  <div className="bg-muted/30 rounded-lg p-4 text-center">
                    <p className="text-muted-foreground text-sm">This user has no token holdings to transfer.</p>
                  </div>
                ) : (
                      ...
                      {fromUserHoldings.map((holding) => (
                        <SelectItem key={holding.token_definition_id} value={holding.token_definition_id}>
                          <span className="flex items-center gap-3">
                            <span className="font-mono text-primary">{holding.token_definition.token_symbol}</span>
            ...
            {selectedHolding && (
              <div className="bg-muted/30 rounded-lg p-4 border border-border">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold text-foreground">{selectedHolding.token_definition.token_name}</p>
                    <p className="text-sm text-primary font-mono">{selectedHolding.token_definition.token_symbol}</p>
                  </div>
            ...
            {fromUser && toUser && selectedHolding && amount && (
              <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                <p className="text-sm text-foreground">
                  Transfer <span className="font-mono font-bold">{parseFloat(amount).toLocaleString()}</span>{' '}
                  <span className="text-primary font-medium">{selectedHolding.token_definition.token_symbol}</span> from{' '}
