...
export default function TrainingPage() {
  ...
  const fetchCourses = async () => {
    try {
      // Fetch published courses - for unauthenticated users, only show public courses
      let query = supabase
      ...
      const { data: coursesData, error: coursesError } = await query as { data: TrainingCourse[] | null; error: any };
  ...
  // Use PublicCourseLayout for unauthenticated users
  if (!user) {
    return (
      <PublicCourseLayout
        title="Training Center"
        subtitle="Learn about precious metal tokenization and earn MXG rewards"
      >
        {content}
      </PublicCourseLayout>
    );
